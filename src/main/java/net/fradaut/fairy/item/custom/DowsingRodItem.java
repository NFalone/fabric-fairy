package net.fradaut.fairy.item.custom;
...
public class DowsingRodItem extends Item {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if(Screen.hasShiftDown()){
            tooltip.add(new TranslatableText("item.fairy.dowsing_rod.tooltip.shift"));
        }else{
            tooltip.add(new TranslatableText("item.fairy.dowsing_rod.tooltip"));
        }
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        if(context.getWorld().isClient()){
            ...
            if(!foundBlock){
                player.sendMessage(new TranslatableText("item.fairy.dowsing_rod.no_valuables"), false);
            }
        }
